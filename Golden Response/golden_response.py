...
WORKSPACE_FILES matrix below, separated into frontend and backend sections, then
written into a clean workspace and validated with a lightweight architectural
harness.
...
WORKSPACE_FILES = {
    ...
    # backend/README.md
    # -------------------------------------------------------------------------
    "backend/README.md": r'''# EDMS Server
...
''',
    ...
    # backend/app.js
    # -------------------------------------------------------------------------
    "backend/app.js": r'''const path = require('path');
...
''',
    ...
    # -------------------------------------------------------------------------
    "backend/config/db.js": r'''const dns = require('dns');
...
''',
    ...
    # -------------------------------------------------------------------------
    "backend/controllers/analyticsController.js": r'''const Employee = require('../models/Employee');
...
''',
    ...
    # -------------------------------------------------------------------------
    "backend/controllers/authController.js": r'''const crypto = require('crypto');
...
''',
    ...
    # -------------------------------------------------------------------------
    "backend/controllers/employeeController.js": r'''const path = require('path');
...
''',
    ...
    # -------------------------------------------------------------------------
    "backend/controllers/notificationController.js": r'''const asyncHandler = require('../utils/asyncHandler');
const { sendSuccess } = require('../utils/http');
...
''',
    ...
    # backend/index.js
    # -------------------------------------------------------------------------
    "backend/index.js": r'''const path = require('path');
...
''',
    ...
    # -------------------------------------------------------------------------
    "backend/middleware/errorHandler.js": r'''const multer = require('multer');
...
''',
    ...
    # -------------------------------------------------------------------------
    "backend/middleware/upload.js": r'''const fs = require('fs');
...
''',
    ...
    # -------------------------------------------------------------------------
    "backend/middleware/validateRequest.js": r'''const { validationResult } = require('express-validator');
...
''',
    ...
    # -------------------------------------------------------------------------
    "backend/models/Employee.js": r'''const mongoose = require('mongoose');
...
''',
    ...
    # -------------------------------------------------------------------------
    "backend/routes/analyticsRoutes.js": r'''const express = require('express');
...
''',
    ...
    # -------------------------------------------------------------------------
    "backend/routes/authRoutes.js": r'''const express = require('express');
const rateLimit = require('express-rate-limit');
...
''',
    ...
    # -------------------------------------------------------------------------
    "backend/routes/employeeRoutes.js": r'''const express = require('express');
...
''',
    ...
    # -------------------------------------------------------------------------
    "backend/routes/notificationRoutes.js": r'''const express = require('express');
const { body } = require('express-validator');
...
''',
    ...
    # -------------------------------------------------------------------------
    "backend/services/emailService.js": r'''const nodemailer = require('nodemailer');
...
''',
    ...
    # -------------------------------------------------------------------------
    "backend/utils/AppError.js": r'''class AppError extends Error {
...
''',
    ...
    # -------------------------------------------------------------------------
    "backend/utils/asyncHandler.js": r'''const asyncHandler = (fn) => (req, res, next) => {
...
''',
    ...
    # -------------------------------------------------------------------------
    "backend/utils/http.js": r'''const sendSuccess = (res, message, data = null, statusCode = 200) => {
  const payload = {
    success: true,
    message
  };
...
''',
    ...
    # -------------------------------------------------------------------------
    "backend/utils/logger.js": r'''const winston = require('winston');
...
''',
    ...
    # -------------------------------------------------------------------------
    "backend/utils/tokens.js": r'''const crypto = require('crypto');
...
''',
    ...
    # frontend/README.md
    # -------------------------------------------------------------------------
    "frontend/README.md": r'''# EDMS Client
...
''',
    ...
    # -------------------------------------------------------------------------
    "frontend/src/App.jsx": r'''import React, { useState } from 'react';
...
''',
    ...
    # -------------------------------------------------------------------------
    "frontend/src/components/AuthScreen.jsx": r'''import React, { useState } from 'react';
...
''',
    ...
    # -------------------------------------------------------------------------
    # frontend/src/components/ContactHrModal.jsx
    # -------------------------------------------------------------------------
    "frontend/src/components/ContactHrModal.jsx": r'''import React, { useEffect, useState } from 'react';
import { AnimatePresence, motion } from 'framer-motion';
...
''',
    ...
    # -------------------------------------------------------------------------
    "frontend/src/components/DashboardView.jsx": r'''import React from 'react';
import { motion } from 'framer-motion';
import {
  Area,
  AreaChart,
...
''',
    ...
    # -------------------------------------------------------------------------
    # frontend/src/components/EmployeeDrawer.jsx
    # -------------------------------------------------------------------------
    "frontend/src/components/EmployeeDrawer.jsx": r'''import React from 'react';
import { AnimatePresence, motion } from 'framer-motion';
...
''',
    ...
    # -------------------------------------------------------------------------
    "frontend/src/components/EmployeeTable.jsx": r'''import React, { startTransition, useDeferredValue, useState } from 'react';
import { motion } from 'framer-motion';
...
''',
    ...
    # -------------------------------------------------------------------------
    "frontend/src/components/Sidebar.jsx": r'''import React from 'react';
import { motion } from 'framer-motion';
...
''',
    ...
    # -------------------------------------------------------------------------
    "frontend/src/context/AppContext.jsx": r'''import React, { createContext, startTransition, useContext, useEffect, useState } from 'react';
...
''',
    ...
    # -------------------------------------------------------------------------
    "frontend/src/data/mockData.js": r'''export const mockCurrentUser = {
  _id: 'user-1',
  name: 'Avery Carter',
...
''',
    ...
    # -------------------------------------------------------------------------
    "frontend/src/index.css": r'''@import url('https://fonts.googleapis.com/css2?family=Fraunces:wght@600;700&family=Space+Grotesk:wght@400;500;700&display=swap');
...
''',
    ...
    # -------------------------------------------------------------------------
    "frontend/src/lib/api.js": r'''import axios from 'axios';
...
def generate_workspace(base_dir: str | os.PathLike[str] | None = None) -> None:
    """
    Materializes the serialized project files into the Golden Response folder.
...
def run_test_harness_suite() -> None:
    ...
    backend_package = json.loads(WORKSPACE_FILES["backend/package.json"])
    backend_deps = backend_package.get("dependencies", {})
    for dependency in ["bcryptjs", "cors", "dotenv", "express", "helmet", "jsonwebtoken", "mongoose", "multer", "nodemailer"]:
        assert dependency in backend_deps, f"Missing backend dependency: {dependency}"
    print("[OK] Test Suite Validation Module 2: Backend dependency matrix confirmed.")

    auth_middleware_code = WORKSPACE_FILES["backend/middleware/auth.js"]
    ...
    employee_routes_code = WORKSPACE_FILES["backend/routes/employeeRoutes.js"]
    ...
    frontend_package = json.loads(WORKSPACE_FILES["frontend/package.json"])
    frontend_deps = frontend_package.get("dependencies", {})
    assert "framer-motion" in frontend_deps, "Frontend must include Framer Motion for animated UI flows."
    assert "recharts" in frontend_deps, "Frontend must include charting support for analytics dashboards."
